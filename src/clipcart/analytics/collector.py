...
def build_snapshot(
    ...
    # 채널 시청 유지율 — 중앙값(유지율 있는 영상만). 쇼츠 루프/API 글리치로 한 영상이
    # 3144% 같은 손상값을 줘도 흔들리지 않게 평균 대신 중앙값을 쓴다.
    ret_vals = [
        v["avg_view_pct"] for v in videos if v.get("avg_view_pct") is not None and v["views"]
    ]
    avg_view_pct = round(statistics.median(ret_vals), 1) if ret_vals else None
    return {
