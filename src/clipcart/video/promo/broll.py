...
def _story_broll(q: dict[str, str]) -> dict[str, str]:
    """story: 과장된 더러움 대신 밝고 화사한 톤. pain도 '정돈 안 된 한구석' 수준으로."""
    return {
        "pain": f"soft natural light bright airy modern home, {q['pain']}",
        "use": f"soft natural window light, {q['use']}",
        "clean": f"bright airy soft light, {q['clean']}",
    }


def get_broll(niche: dict) -> dict[str, str]:
    q = BROLL.get(niche.get("keyword", "")) or _CATEGORY_FALLBACK.get(
        niche.get("category", ""),
        {"pain": "messy home", "use": "cleaning home", "clean": "clean tidy home"},
    )
    return _story_broll(q) if is_story() else q
