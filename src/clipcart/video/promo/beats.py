...
def build_beats(product: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    # 두 여성 목소리 — 증언 보이스는 '앞부분 problem'에서만(운영자 지시 2026-06-22):
    # 끝 CTA를 증언 보이스가 하면('댓글 확인하세요') 어색해서 메인으로 되돌리고,
    # 공감되는 문제 장면(개인 관찰 화법)을 증언 보이스로 옮긴다. 일반·관찰 화법이라
    # 허위후기 아님(컴플라이언스가 차단). 미설정(env) 시 메인으로 폴백돼 단일 보이스.
    for b in beats:
        if b["role"] == "problem":
            b["voice"] = "testimony"
