...
## 0.5 전자동 모드 (AUTO MODE — 2026-06-10 운영자 지시)

운영자는 YouTube Shorts 데일리 파이프라인에 한해 **포괄 사전 승인**을 부여했다.
TopView 수동 제작 단계는 내장 영상 엔진으로 대체되었다.

```text
clipcart daily --live
```

위 명령 한 번으로 다음이 전자동 실행된다:
...
* 1.2 금지 행동 전부
* 컴플라이언스 게이트 (자동 검사가 게시 차단 권한을 가진 하드 게이트)
* Instagram/TikTok 게시, 광고비, 신규 카테고리 진입은 여전히 사람 승인 필요

API로 불가능해 사람이 해야 하는 것:

* YouTube Studio에서 '유료 프로모션 포함' 체크 (업로드 후)
* 고정댓글 등록 (파이프라인이 pinned_comment_suggestion 제공)
...
공정위 2024-12 개정 지침: 조건부 표현("받을 수 있습니다") 금지, 확정형만 인정.

```text
이 포스팅은 쿠팡 파트너스 활동의 일환으로, 이에 따른 일정액의 수수료를 제공받습니다.
```

고지 위치: 설명란 첫 부분(더보기 접힘 위) + 영상 시작/끝 자막. 끝부분만 표기는 불인정.

짧은 플랫폼 문구 (보조용, 단독 사용 금지):
